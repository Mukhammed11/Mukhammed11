...
class Thor(Hero):
    ...
    def apply_super_power(self, boss, heroes):
        chance = randint(1, 11) # 1, 2, 3, 4, ... , 10
        if chance == 1:
            boss.damage = 0
...
class Reaper(Hero):
    def __init__(self, name, health, damage):
        super().__init__(name, health, damage, Ability.DAMAGE_BOOST)
    ...
    def apply_super_power(self, boss, heroes):
        if self.health <= self.initial_health * 0.3:
            ...
            print(f"{self.name} увеличил свой урок вдвое")
        elif self.health <= self.initial_health * 0.15:
            self.damage = self.damage * 3
            print(f"{self.name} увеличил свой урок втрое")
    ...
    def apply_super_power(self, boss, heroes):
        for hero in heroes:
            if hero.health <= 0:
                ...
                self.health = 0
                print(f"{self.name} возродил {hero.name}")
                break
...
def start_game():
    ...
    reaper = Reaper('Death', 210, 10)
    witcher = Witcher('Savada', 300, 0)

    heroes_list = [warrior_1, warrior_2, magic, doc, berserk, assistant, thor, reaper, witcher]

    boss = Boss('Baron Nashor', 2500, 60)
